...
void sjf_proc(Process process[],int n)
{
    int time=0;
    int complete=0;
    ...
    while(complete!=n)
    {
        ...
        for(int i=0;i<n;i++)
        {
            if(process[i].completed==false && process[i].arrival_time<=time && process[i].burst_time<min_val)
            {
                min_val=process[i].burst_time;
                shortest=i;
            }
        }
        ...
        process[shortest].response_time=process[shortest].waiting_time;
        ...
        time+=process[shortest].burst_time;
